...
export class FieldResonatorMock {
  ...
  /**
   * Simulate resonance pulse (async to support real plugin data)
   */
  async simulatePulse(efficiency: Float64Array): Promise<{
    signal: SignalFrame;
    fieldState: FieldState;
    recommendation: string;
  }> {
    const signal = await generateSignalFrame(efficiency);
