...
export interface TelemetryStreamSummary {
  ...
  vendorPayload: Array<{ vendor: string; payload: any[]; meta?: any[] }>
  recentActions: Array<{
    timestamp: string
    actionType: string
    success: boolean
...
export interface TelemetrySummary {
  ...
  lastUpdated: string | null
  eventCounts: Array<{ type: TelemetryEvent['type']; count: number }>
  vendorSpend: Array<{ vendor: string; payload: any[]; meta?: any[] }>
  recentActions: Array<{
    timestamp: string
    actionType: string
    success: boolean
...
function summariseVendorPayload(events: TelemetryEvent[]): Array<{ vendor: string; payload: any[]; meta: any[] }> {
  // Group events by vendor
  const vendorMap = new Map<string, { payload: any[]; meta: any[] }>();
  for (const event of events) {
    // Example: extract vendor from event.agentId or event.payload.vendor
    const vendor =
      (event.payload && 'vendor' in event.payload && typeof (event.payload as any).vendor === 'string' && (event.payload as any).vendor) ||
      (typeof event.agentId === 'string' ? event.agentId : 'unknown');
    if (!vendorMap.has(vendor)) {
      vendorMap.set(vendor, { payload: [], meta: [] });
    }
    vendorMap.get(vendor)!.payload.push(event.payload);
    vendorMap.get(vendor)!.meta.push(event.provenance ?? {});
  }
  return Array.from(vendorMap.entries()).map(([vendor, { payload, meta }]) => ({ vendor, payload, meta }));
}
export async function getTelemetrySummary(limit = 400): Promise<TelemetrySummary> {
  ...
  return {
    ...
    vendorSpend: summariseVendorPayload(events),
    ...overall,
    streams: {
      demo,
      gameboard,
...
function buildStreamSummary(events: TelemetryEvent[], lastUpdated: string | null): TelemetryStreamSummary {
  ...
  return {
    ...
    eventCounts,
    vendorPayload: summariseVendorPayload(events),
...
function describeEvent(event: TelemetryEvent) {
  switch (event.type) {
    ...
    case 'payload.failed':
      return `Payload failed: ${event.payload.error}`
    case 'payload.delta':
      return `Payload updated → remaining ${event.payload.remaining.toLocaleString()} lamports`
    case 'agent.halted':
...
function isBudgetDeltaEvent(event: TelemetryEvent): event is BudgetDeltaEvent {
  return event.type === 'payload.delta'
}
