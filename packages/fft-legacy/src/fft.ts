...
export function fftFunc(vector: (number | Complex)[]): Complex[] {
  const N = vector.length;
  const X: Complex[] = [];

  if (N === 1) {
    const val = vector[0];
    return [Array.isArray(val) ? [val[0], val[1]] : [Number(val), 0]];
  }
...
function magnitudeFromComplex(values: Complex[]): Float64Array {
...
function padToPowerOfTwo(realData: Float64Array): Float64Array {
  const length = realData.length;
  if (length === 0) {
    return new Float64Array(0);
  }

  const isPowerOfTwo = (length & (length - 1)) === 0;
  if (isPowerOfTwo) {
    return realData;
  }
...
export function forwardMagnitudes(realData: Float64Array): Float64Array {
  const normalized = padToPowerOfTwo(realData);
  const spectrum = fftFunc(Array.from(normalized));
  return magnitudeFromComplex(spectrum);
}

export function deriveCoherence(realData: Float64Array): number {
